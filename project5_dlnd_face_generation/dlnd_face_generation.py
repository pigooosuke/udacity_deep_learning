...
def discriminator(images, reuse=False):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        conv3 =  tf.reshape(conv3, (-1,4*4*256))
...
def generator(z, out_channel_dim, is_train=True):
    ...
    with tf.variable_scope("generator", reuse=(not is_train)):
        ...
        conv1 = tf.layers.conv2d_transpose(z, 256, 4, strides=1, padding="valid")
        conv1 = tf.layers.batch_normalization(conv1, training=is_train)
        conv1 = lrelu(conv1, alpha)
        conv1 = tf.layers.dropout(conv1, rate=0.2, training=is_train)
        ...
        logits = tf.layers.conv2d_transpose(conv2, out_channel_dim, 4, strides=2, padding="same")
